...
function doGet(e) {
  return HtmlService
    .createHtmlOutput(buildHtml_(e))
    .setTitle("IMDB — Campagne & clienti");
}
...
function apiGetLogStatus() {
  return safe_("apiGetLogStatus", function () {
    var t = logGet_();
    return { ok: true, hasLog: !!t, length: t.length };
  });
}

function apiGetLog() {
  return safe_("apiGetLog", function () {
    return { ok: true, text: logGet_() };
  });
}

function apiClearLog() {
  return safe_("apiClearLog", function () {
    logClear_();
    return { ok: true };
  });
}
...
function apiGetCampaigns() {
  return safe_("apiGetCampaigns", function () {
    return getCampaigns_();
  });
}

function apiGetHeadersForCampaign(campaignSheetName) {
  return safe_("apiGetHeadersForCampaign", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    return { headers: headers };
  });
}

function apiGetClients(campaignSheetName) {
  return safe_("apiGetClients", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);
    var lastCol = sh.getLastColumn();

    var cCognome = colOfExact_(idx, ANCHORS.cognome);
    if (!cCognome) {
      logAppend_('Manca colonna obbligatoria "' + ANCHORS.cognome + '" (riga ' + APP_CFG.headerRow + ') in sheet "' + sh.getName() + '"');
      logAppend_("Headers rilevati: " + JSON.stringify(headers));
      return { ok: false, error: 'Manca la colonna obbligatoria "' + ANCHORS.cognome + '" (riga ' + APP_CFG.headerRow + ")." };
    }

    var cNome = colOfExact_(idx, ANCHORS.nome);
    var cMail = colOfExact_(idx, ANCHORS.email);
    var cTel = colOfExact_(idx, ANCHORS.telefono);
    var cId = colOfExact_(idx, ANCHORS.idCliente);

    var rows = getClientRowIndexes_(sh);
    var clients = rows.map(function (r) {
      var rowVals = sh.getRange(r, 1, 1, lastCol).getDisplayValues()[0];

      var cognome = String(rowVals[cCognome - 1] || "").trim();
      var nome = cNome ? String(rowVals[cNome - 1] || "").trim() : "";
      var mail = cMail ? String(rowVals[cMail - 1] || "").trim() : "";
      var tel = cTel ? String(rowVals[cTel - 1] || "").trim() : "";
      var idc = cId ? String(rowVals[cId - 1] || "").trim() : "";
      ...
      return {
        ...
        idCliente: idc,
        _k: {
          cognome: IMDBCommonLibs.normText(cognome),
          mail: IMDBCommonLibs.normText(mail),
          id: IMDBCommonLibs.normText(idc),
          tel: IMDBCommonLibs.normPhone(tel)
        },
        display: (String((cognome + " " + nome).trim()) + (mail ? " — " + mail : "")).trim() || ("Riga " + r)
      };
    });
...
function apiGetClientDetails(campaignSheetName, rowIndex) {
  return safe_("apiGetClientDetails", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);
    var lastCol = sh.getLastColumn();
    ...
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiGetClientDetails: " + rowIndex + " camp=" + campaignSheetName);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }

    var rowVals = sh.getRange(rowIndex, 1, 1, lastCol).getDisplayValues()[0];
    var details = buildClientDetails_(sh, headers, idx, rowVals, rowIndex, null);
    return { ok: true, details: details };
  });
}

function apiGetClientDetails(campaignSheetName) {
  return safe_("apiGetClientDetails", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);
    ...
    var rowIndexes = getClientRowIndexes_(sh);
    if (!rowIndexes.length) return { ok: true, rows: {}, rowIndexes: [], cachedAt: new Date().toISOString() };

    var first = rowIndexes[0];
    var last = rowIndexes[rowIndexes.length - 1];
    var numRows = last - first + 1;

    var values = sh.getRange(first, 1, numRows, lastCol).getDisplayValues();
    var bottleCols = getBottleColsByRow1Rule_(sh);

    var out = {};
    for (var i = 0; i < values.length; i++) {
      var rowIndex = first + i;
      var rowVals = values[i];

      var colA = String(rowVals[APP_CFG.colA - 1] || "").trim().toLowerCase();
      if (colA.indexOf("cliente") !== 0) continue;

      out[String(rowIndex)] = buildClientDetails_(sh, headers, idx, rowVals, rowIndex, bottleCols);
    }

    return { ok: true, rows: out, rowIndexes: rowIndexes, cachedAt: new Date().toISOString() };
  });
}

function apiSetCellByHeader(campaignSheetName, rowIndex, headerNameExact, value) {
  return safe_("apiSetCellByHeader", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);

    rowIndex = Number(rowIndex);
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiSetCellByHeader: " + rowIndex + " camp=" + campaignSheetName);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }

    var targetHeader = String(headerNameExact || "").trim();
    if (!targetHeader) {
      logAppend_("headerName mancante in apiSetCellByHeader (camp=" + campaignSheetName + ", row=" + rowIndex + ")");
      return { ok: false, error: "headerName mancante." };
    }

    var colIndex = findFirstHeaderCol_(headers, targetHeader);
    if (!colIndex) {
      logAppend_('Header non trovato in riga ' + APP_CFG.headerRow + ': "' + targetHeader + '"');
      logAppend_("Headers rilevati: " + JSON.stringify(headers));
      return { ok: false, error: 'Header non trovato in riga ' + APP_CFG.headerRow + ': "' + targetHeader + '"' };
    }

    sh.getRange(rowIndex, colIndex).setValue(String(value == null ? "" : value));
    SpreadsheetApp.flush();

    return { ok: true, colIndex: colIndex };
  });
}

function apiSetBottleQtyAndGetOrderBase(campaignSheetName, rowIndex, colIndex, qty) {
  return safe_("apiSetBottleQtyAndGetOrderBase", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);
    var lastCol = sh.getLastColumn();
    ...
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiSetBottleQtyAndGetOrderBase: " + rowIndex);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }
    if (!colIndex || colIndex < 1) {
      logAppend_("colIndex non valido in apiSetBottleQtyAndGetOrderBase: " + colIndex);
      return { ok: false, error: "colIndex non valido: " + colIndex };
    }

    var q = String(qty == null ? "" : qty).trim();
    if (q === "") q = "0";

    sh.getRange(rowIndex, colIndex).setValue(q);
    SpreadsheetApp.flush();

    var rowVals = sh.getRange(rowIndex, 1, 1, lastCol).getDisplayValues()[0];
    var ordineBase = buildOrdineBase_(headers, idx, rowVals);

    return { ok: true, ordineBase: ordineBase };
  });
}

function apiSetBottleQtyBulkAndGetOrderBase(campaignSheetName, rowIndex, items) {
  return safe_("apiSetBottleQtyBulkAndGetOrderBase", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);
    var lastCol = sh.getLastColumn();
    ...
    rowIndex = Number(rowIndex);
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiSetBottleQtyBulkAndGetOrderBase: " + rowIndex);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }

    var list = Array.isArray(items) ? items : [];
    list.forEach(function (it) {
      var col = Number(it && it.col);
      if (!col || col < 1) return;

      var raw = (it && it.qty !== undefined && it.qty !== null) ? it.qty : "";
      var q = String(raw).trim();
      if (q === "") q = "0";

      sh.getRange(rowIndex, col).setValue(q);
    });
...
function apiRunMacro(campaignSheetName, rowIndex, macroName) {
  return safe_("apiRunMacro", function () {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sh = getCampaignSheet_(campaignSheetName);

    rowIndex = Number(rowIndex);
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiRunMacro: " + rowIndex);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }

    var macro = String(macroName || "").trim();
    if (!macro) {
      logAppend_("macroName mancante in apiRunMacro");
      return { ok: false, error: "macroName mancante." };
    }

    ss.setActiveSheet(sh);
    var rng = sh.getRange(rowIndex, 1, 1, sh.getLastColumn());
    sh.setActiveRange(rng);

    var fn = this[macro];
    if (typeof fn !== "function") {
      logAppend_("Macro non trovata: " + macro);
      return { ok: false, error: "Macro non trovata: " + macro };
    }

    try {
      if (fn.length >= 2) fn(campaignSheetName, rowIndex);
      else if (fn.length === 1) fn(rowIndex);
      else fn();
    } catch (e) {
      logError_("Macro:" + macro, e, { campaignSheetName: campaignSheetName, rowIndex: rowIndex });
      return { ok: false, error: "Errore in " + macro + ": " + (e && e.message ? e.message : e) };
    }
...
function apiFindMauticCustomerData(query) {
  return safe_("apiFindMauticCustomerData", function () {
    var q = String(query || "").trim();
    if (!q) return { ok: true, found: false, results: [] };

    if (!this.IMDBCommonLibs || typeof IMDBCommonLibs.getMauticCustomerData !== "function") {
      logAppend_("IMDBCommonLibs.getMauticCustomerData() non disponibile in questo progetto.");
      return { ok: false, error: "IMDBCommonLibs.getMauticCustomerData() non disponibile in questo progetto." };
    }

    if (!IMDBCommonLibs || typeof IMDBCommonLibs.normalizeMauticCustomerData !== "function") {
      logAppend_("IMDBCommonLibs.normalizeMauticCustomerData() non disponibile.");
      return { ok: false, error: "IMDBCommonLibs.normalizeMauticCustomerData() non disponibile." };
    }

    var raw = IMDBCommonLibs.getMauticCustomerData(q);
    if (!raw) return { ok: true, found: false, results: [] };

    var candidates = IMDBCommonLibs.extractMauticCandidates(raw);
    var results = [];

    for (var i = 0; i < candidates.length; i++) {
      var norm = IMDBCommonLibs.normalizeMauticCustomerData(candidates[i]);
      if (!norm) continue;

      if (IMDBCommonLibs.isEmptyPlainObject && IMDBCommonLibs.isEmptyPlainObject(norm)) continue;

      var labelParts = [];
      var full = String(norm.fullname || "").trim();
      var email = String(norm.email || "").trim();
      var phone = String(norm.phone || "").trim();
      var id = String(norm.id || "").trim();

      if (full) labelParts.push(full);
      if (email) labelParts.push(email);
      if (phone) labelParts.push(phone);
      if (id) labelParts.push("ID " + id);

      results.push({
        value: String(i),
        label: labelParts.join(" — ") || ("Risultato " + (i + 1)),
        data: norm
      });
    }

    if (!results.length) {
      logAppend_("Ricerca Mautic senza risultati utili per query: " + q);
...
function apiCreateNewOrderRow(campaignSheetName, baseFields) {
  return safe_("apiCreateNewOrderRow", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    var headers = getHeaderRow_(sh);
    var idx = buildHeaderIndexExact_(headers);

    var rows = getClientRowIndexes_(sh);
    var insertAt = rows.length ? (rows[rows.length - 1] + 1) : APP_CFG.firstClientRow;

    sh.insertRowBefore(insertAt);
    sh.getRange(insertAt, APP_CFG.colA).setValue("Cliente");

    var obj = (baseFields && typeof baseFields === "object") ? baseFields : {};
    Object.keys(obj).forEach(function (h) {
      var col = colOfExact_(idx, h);
      if (!col) return;
      sh.getRange(insertAt, col).setValue(String(obj[h] == null ? "" : obj[h]));
    });

    var prevRow = insertAt - 1;
    if (prevRow >= APP_CFG.firstClientRow) {
      var cStart = findFirstHeaderCol_(headers, ANCHORS.formulaCopyStartHeader);
      var cEnd = findFirstHeaderCol_(headers, ANCHORS.formulaCopyEndHeader);
      if (!cStart) {
        logAppend_('Header non trovato (riga ' + APP_CFG.headerRow + '): "' + ANCHORS.formulaCopyStartHeader + '"');
        return { ok: false, error: 'Header non trovato (riga ' + APP_CFG.headerRow + '): "' + ANCHORS.formulaCopyStartHeader + '"' };
      }
      if (!cEnd) {
        logAppend_('Header non trovato (riga ' + APP_CFG.headerRow + '): "' + ANCHORS.formulaCopyEndHeader + '"');
        return { ok: false, error: 'Header non trovato (riga ' + APP_CFG.headerRow + '): "' + ANCHORS.formulaCopyEndHeader + '"' };
      }
      if (cEnd < cStart) {
        logAppend_("Range formule non valido: " + ANCHORS.formulaCopyStartHeader + " -> " + ANCHORS.formulaCopyEndHeader);
        return { ok: false, error: "Range formule non valido: " + ANCHORS.formulaCopyStartHeader + " -> " + ANCHORS.formulaCopyEndHeader };
      }

      var width = cEnd - cStart + 1;
      var src = sh.getRange(prevRow, cStart, 1, width);
      var dst = sh.getRange(insertAt, cStart, 1, width);

      var formulas = src.getFormulasR1C1()[0];
      dst.setFormulasR1C1([formulas.map(function (f) { return f || ""; })]);
    }

    SpreadsheetApp.flush();

    var lastCol = sh.getLastColumn();
    var rowVals = sh.getRange(insertAt, 1, 1, lastCol).getDisplayValues()[0];
    var details = buildClientDetails_(sh, headers, idx, rowVals, insertAt, null);

    return { ok: true, rowIndex: insertAt, details: details };
  });
}

function apiDeleteRow(campaignSheetName, rowIndex) {
  return safe_("apiDeleteRow", function () {
    var sh = getCampaignSheet_(campaignSheetName);
    rowIndex = Number(rowIndex);
    if (!rowIndex || rowIndex < APP_CFG.firstClientRow) {
      logAppend_("rowIndex non valido in apiDeleteRow: " + rowIndex);
      return { ok: false, error: "rowIndex non valido: " + rowIndex };
    }
...
function onOpen(e) {
  try {
    ...
    ensureIdClienteLinksForSheet_(sh);
  } catch (err) {
    logError_("onOpen", err, null);
  }
}
...
function onEdit(e) {
  try {
    ...
    if (range.getColumn() !== cId) return;
    if (range.getRow() < APP_CFG.firstClientRow) return;

    ensureIdClienteLinksForRange_(sh, range);
  } catch (err) {
    logError_("onEdit", err, null);
  }
}*/

/* =========================
 * Backend helpers (app-specific)
 * ========================= */

function ensureIdClienteLinks_() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ss.getSheets().forEach(function (sh) {
    try {
      ensureIdClienteLinksForSheet_(sh);
    } catch (err) {
      logError_("ensureIdClienteLinks_", err, { sheet: sh.getName() });
    }
  });
}

function ensureIdClienteLinksForSheet_(sh) {
  var headers = getHeaderRow_(sh);
  var idx = buildHeaderIndexExact_(headers);
  var cId = colOfExact_(idx, ANCHORS.idCliente);
  if (!cId) return;
  ...
  var rng = sh.getRange(rows[0], cId, rows.length, 1);
  ensureIdClienteLinksForRange_(sh, rng);
}

function ensureIdClienteLinksForRange_(sh, rng) {
  ...
  for (var i = 0; i < numRows; i++) {
    var r = startRow + i;
    if (r < APP_CFG.firstClientRow) continue;

    var idText = String(vals[i][0] || "").trim();
    if (!idText) continue;

    var cell = sh.getRange(r, col);
    if (cell.getFormula()) continue;

    var targetUrl = buildWebAppLinkByRow_(webAppUrl, sh.getName(), r);

    var current = rich[i][0];
    var currentUrl = current && current.getLinkUrl ? current.getLinkUrl() : null;
    if (currentUrl === targetUrl) continue;
...
function getWebAppUrl_() {
  var u = String(PropertiesService.getScriptProperties().getProperty("IMDB_WEBAPP_URL") || "").trim();
  if (!u) {
    logAppend_("Script property IMDB_WEBAPP_URL mancante (metti l'URL /exec della Web App).");
    throw new Error("Script property IMDB_WEBAPP_URL mancante (metti l'URL /exec della Web App).");
  }
  return u;
}

function buildWebAppLinkByRow_(webAppUrl, campaignSheetName, rowIndex) {
  var base = String(webAppUrl || "").replace(/\/+$/, "");
  var camp = encodeURIComponent(String(campaignSheetName || "").trim());
  var row = encodeURIComponent(String(rowIndex || "").trim());
  return base + "?camp=" + camp + "&row=" + row;
}

function buildClientDetails_(sh, headers, idx, rowVals, rowIndex, preBottleCols) {
  var anagrafica = buildAnagrafica_(headers, idx, rowVals);
  var ordineBase = buildOrdineBase_(headers, idx, rowVals);

  var bottleCols = preBottleCols || getBottleColsByRow1Rule_(sh);
  var bottiglie = buildBottiglieFromCols_(bottleCols, rowVals);

  var datiAggiuntiviOrdine = pickRangeOrdered_(headers, rowVals, ANCHORS.ordineRangeStart, ANCHORS.ordineRangeEnd);
  var sheetRowUrl = buildSheetRowUrl_(sh, rowIndex);

  return {
    anagrafica: anagrafica,
    ordineBase: ordineBase,
    bottiglie: bottiglie,
    datiAggiuntiviOrdine: datiAggiuntiviOrdine,
    _debug: { rowIndex: rowIndex },
    _nav: { sheetRowUrl: sheetRowUrl }
  };
}

function buildSheetRowUrl_(sh, rowIndex) {
  var ssId = SpreadsheetApp.getActiveSpreadsheet().getId();
  var gid = sh.getSheetId();
  return "https://docs.google.com/spreadsheets/d/" + ssId + "/edit#gid=" + gid + "&range=A" + rowIndex;
}

function buildAnagrafica_(headers, idx, rowVals) {
  var required = [
    ANCHORS.idCliente, ANCHORS.cognome, ANCHORS.nome, ANCHORS.vip,
    ANCHORS.email, ANCHORS.telefono, ANCHORS.codiceFiscale, ANCHORS.ad,
    ANCHORS.statoPagamento, ANCHORS.note
  ];

  required.forEach(function (h) {
    if (!colOfExact_(idx, h)) {
      logAppend_('Manca colonna in riga ' + APP_CFG.headerRow + ': "' + h + '" (Anagrafica)');
    }
  });

  var out = {};
  required.forEach(function (h) {
    var c = colOfExact_(idx, h);
    out[h] = c ? String(rowVals[c - 1] || "").trim() : "";
  });

  return out;
}

function buildOrdineBase_(headers, idx, rowVals) {
  var out = {};
  ANCHORS.ordineBaseFields.forEach(function (h) {
    var c = colOfExact_(idx, h);
    if (!c) {
      logAppend_('Manca colonna in riga ' + APP_CFG.headerRow + ': "' + h + '" (Ordine base)');
      out[h] = "";
      return;
    }
    out[h] = String(rowVals[c - 1] || "").trim();
  });
  return out;
}
...
function getCampaigns_() {
  var props = PropertiesService.getScriptProperties();
  var raw = String(props.getProperty("IMDB_CAMPAIGN_SHEETS") || "").trim();
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  if (raw) {
    ...
    var sh = ss.getSheetByName("Campagne");
    if (sh) {
      var n = Math.max(0, sh.getLastRow() - 1);
      var vals = n ? sh.getRange(2, 1, n, 1).getDisplayValues().map(function (r) { return r[0]; }) : [];
      names = vals.map(function (s) { return String(s || "").trim(); }).filter(Boolean);
    }
  }
  ...
  return names.filter(function (nm) { return !!existing[nm]; }).map(function (nm) { return { name: nm }; });
}

function getCampaignSheet_(name) {
  name = String(name || "").trim();
  if (!name) {
    logAppend_("Nome campagna mancante.");
    throw new Error("Nome campagna mancante.");
  }
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName(name);
  if (!sh) {
    logAppend_("Sheet campagna non trovato: " + name);
    throw new Error("Sheet campagna non trovato: " + name);
  }
  return sh;
}
...
function getHeaderRow_(sh) {
  var lastCol = sh.getLastColumn();
  return sh.getRange(APP_CFG.headerRow, 1, 1, lastCol).getDisplayValues()[0];
}

function buildHeaderIndexExact_(headers) {
  var map = new Map();
...
function colOfExact_(idxMapExact, exactHeader) {
  if (!exactHeader) {
    logAppend_("colOfExact_: exactHeader nullo o vuoto");
    return 0;
  }

  var key = String(exactHeader).trim().toLowerCase();
  var it = idxMapExact.entries();
  var step = it.next();
  ...
  while (!step.done) {
    ...
    if (mapKey && String(mapKey).trim().toLowerCase() === key) return value;
    step = it.next();
  }

  logAppend_("colOfExact_: header non trovato -> " + exactHeader);
  return 0;
}

function findFirstHeaderCol_(headers, exact) {
  for (var i = 0; i < headers.length; i++) {
    if (String(headers[i] || "").trim() === exact) return i + 1;
  }
  return 0;
}

function findLastHeaderCol_(headers, exact) {
...
function getClientRowIndexes_(sh) {
  var lastRow = sh.getLastRow();
  var out = [];
  for (var r = APP_CFG.firstClientRow; r <= lastRow; r++) {
    var v = String(sh.getRange(r, APP_CFG.colA).getDisplayValue() || "").trim().toLowerCase();
    if (v.indexOf("cliente") !== 0) break;
    out.push(r);
  }
  return out;
}

function pickRangeOrdered_(headers, rowVals, startHeader, endHeader) {
  var cStart = findFirstHeaderCol_(headers, startHeader);
  if (!cStart) {
    logAppend_('Header non trovato (riga ' + APP_CFG.headerRow + '): "' + startHeader + '" (pickRangeOrdered_)');
    return [];
  }

  var cEnd = findLastHeaderCol_(headers, endHeader);
  if (!cEnd) {
    logAppend_('Header non trovato (riga ' + APP_CFG.headerRow + '): "' + endHeader + '" (pickRangeOrdered_)');
    return [];
  }

  if (cEnd < cStart) {
    logAppend_("Range non valido: " + startHeader + " -> " + endHeader);
    return [];
  }
  ...
  for (var c = cStart; c <= cEnd; c++) {
    ...
    if (!h) continue;
    list.push({ header: h, value: String(rowVals[c - 1] || "").trim() });
  }
  return list;
}

function getBottleColsByRow1Rule_(sh) {
  var lastCol = sh.getLastColumn();
  var row1 = sh.getRange(APP_CFG.metaRows.code, 1, 1, lastCol).getDisplayValues()[0];

  var cLabel = 0;
  for (var c = 1; c <= lastCol; c++) {
    if (String(row1[c - 1] || "").trim() === ANCHORS.bottleLabelInRow1) { cLabel = c; break; }
  }
  if (!cLabel) return [];

  var cStart = cLabel + ANCHORS.bottleStartOffset;
  if (cStart > lastCol) return [];

  var cols = [];
  for (c = cStart; c <= lastCol; c++) {
    var codice = String(sh.getRange(APP_CFG.metaRows.code, c).getDisplayValue() || "").trim();
    if (!codice) break;

    cols.push({
      col: c,
      codice_bottiglia: codice,
      produttore: String(sh.getRange(APP_CFG.metaRows.producer, c).getDisplayValue() || "").trim(),
      descrizione: String(sh.getRange(APP_CFG.metaRows.description, c).getDisplayValue() || "").trim()
    });
  }
  return cols;
}

function buildBottiglieFromCols_(bottleCols, clientRowVals) {
  var out = [];
  (bottleCols || []).forEach(function (bc) {
    var qtyRaw = String(clientRowVals[bc.col - 1] || "").trim();
    var qty = qtyRaw ? qtyRaw : "0";
    out.push({
      col: bc.col,
      codice_bottiglia: bc.codice_bottiglia,
      produttore: bc.produttore,
      descrizione: bc.descrizione,
      numero_bottiglie: qty
    });
  });
  return out;
}

function getDeploymentVersion_() {
  try {
    var deployments = ScriptApp.getDeploymentInfo();
    for (var i = 0; i < deployments.length; i++) {
      var d = deployments[i];
      if (d.getDeploymentConfig().getDescription() === "Web app") {
        return d.getDeploymentConfig().getVersionNumber();
      }
    }
  } catch (e) {
    Logger.log(e);
...
function buildHtml_(e) {
  var p = (e && e.parameter) ? e.parameter : {};
  var initial = {
    camp: String(p.camp || "").trim(),
    row: String(p.row || "").trim(),
    version: (typeof getDeploymentVersion_ === "function") ? getDeploymentVersion_() : ""
  };
...
'    table.excel{border:1px solid #cfcfcf;border-collapse:collapse;}' +
'    table.excel th,table.excel td{border:1px solid #cfcfcf;padding:6px 8px;}' +
'    table.excel thead th{background:#f3f3f3;font-weight:700;}' +
'    table.excel tbody tr:nth-child(even) td{background:#fafafa;}' +
'    .qty{width:120px;}' +
'    .editableCell input,.editableCell textarea,.editableCell select{background:#dff5df;border:1px solid #b6e2b6;border-radius:8px;padding:6px 8px;width:100%;box-sizing:border-box;}' +
'    .editableCell textarea{min-height:60px;resize:vertical;}' +
'    .saving{opacity:.75;}' +
'    .btnbar{display:flex;flex-wrap:wrap;gap:8px;align-items:center;}' +
'    .btnbar button{border:1px solid #ddd;background:#fff;border-radius:10px;}' +
'    .btnbar button:hover{background:#f7f7f7;}' +
'    .btnbar button:disabled{opacity:.55;cursor:not-allowed;}' +
'    .modalBack{position:fixed;inset:0;background:rgba(0,0,0,.35);display:none;align-items:center;justify-content:center;padding:16px;z-index:9999;}' +
'    .modal{background:#fff;border-radius:14px;width:min(1100px,96vw);max-height:92vh;overflow:auto;border:1px solid #ddd;box-shadow:0 12px 40px rgba(0,0,0,.2);padding:14px;}' +
'    .modalHeader{display:flex;align-items:center;gap:10px;flex-wrap:wrap;position:sticky;top:0;background:#fff;padding-bottom:10px;border-bottom:1px solid #eee;z-index:1;}' +
...
'    .danger{border-color:#e2b6b6 !important;background:#fff5f5 !important;}' +
'    #btnShowLog{border:1px solid #ddd;background:#eee;border-radius:10px;}' +
'    #btnShowLog.logOn{background:#ffebeb !important;border-color:#e2b6b6 !important;color:#b00000 !important;}' +
'    #newResults{max-width:520px;}' +
'  </style>' +
'</head><body>' +

'  <div class="row" style="align-items:baseline;margin-bottom:8px;">' +
'    <h2 style="margin:0;">IMDB — Campagne & clienti</h2>' +
...
'  <div class="card">' +
'    <div class="row">' +
'      <label><strong>Campagna</strong></label>' +
'      <select id="campaign"></select>' +
'      <span class="spacer"></span>' +
'      <button id="btnShowLog" disabled title="Mostra log server">Mostra LOG</button>' +
'      <span class="muted">Ricerca cliente</span>' +
'      <input id="qCognome" placeholder="Cognome…" />' +
'      <input id="qMail" placeholder="Email…" />' +
'      <input id="qId" placeholder="ID Cliente…" />' +
'      <input id="qTel" placeholder="Telefono…" />' +
'      <button id="btnNewOrder" title="Nuovo ordine">＋ Nuovo ordine</button>' +
'      <button id="btnPrev" title="Cliente precedente">▲</button>' +
'      <button id="btnNext" title="Cliente successivo">▼</button>' +
'      <select id="client" style="min-width:360px;"></select>' +
'    </div>' +
'    <div class="status muted" id="status"></div>' +
'  </div>' +

'  <div class="grid">' +
'    <div class="card"><h3>Anagrafica</h3><table id="tblAnag"></table></div>' +
'    <div class="card"><h3>Ordine (base)</h3><table id="tblOrdBase"></table></div>' +
'  </div>' +

'  <div class="card"><h3>Azioni</h3><div class="btnbar" id="macroBtns"></div><div class="muted" id="cacheInfo" style="margin-top:8px;"></div></div>' +
'  <div class="card"><h3>Bottiglie</h3><div class="muted">Tutti i codici bottiglia (anche qty 0). Qty editabile: al cambio ricarico "Ordine (base)".</div><div id="bottles"></div></div>' +
'  <div class="card"><h3>Dati aggiuntivi ordine</h3><div class="muted">Sequenza come il foglio. "?" => SI/NO, date => editor data, altri => testo.</div><table id="tblOrdRange"></table></div>' +

'  <div class="modalBack" id="modalBack">' +
'    <div class="modal" role="dialog" aria-modal="true" aria-label="Nuovo ordine">' +
'      <div class="modalHeader">' +
'        <strong>Nuovo ordine</strong>' +
'        <span class="muted" id="modalStatus"></span>' +
'        <span class="spacer"></span>' +
'        <button id="btnModalClose">Chiudi</button>' +
'      </div>' +
'      <div class="card" style="margin-top:12px;">' +
'        <div class="row">' +
'          <span class="muted">ID Cliente / Cognome / Email</span>' +
'          <input id="newQuery" placeholder="Scrivi e premi Enter…" style="min-width:320px;" />' +
'          <button id="btnSearchMautic" type="button">Cerca</button>' +
'          <select id="newResults" style="min-width:360px;display:none;"></select>' +
'          <button id="btnUseSelectedMautic" type="button" style="display:none;">Usa selezionato</button>' +
'          <span class="muted" id="newRowInfo"></span>' +
'        </div>' +
'      </div>' +
'      <div class="modalGrid">' +
'        <div class="card"><h3>Anagrafica (Mautic)</h3><div class="muted">Se più risultati coincidono, scegli quello corretto.</div><table id="tblNewAnag"></table></div>' +
'        <div class="card"><h3>Ordine (base)</h3><div class="muted">Editabili: Prezzo, Spedizione, Voucher</div><table id="tblNewOrdBase"></table></div>' +
'      </div>' +
'      <div class="card"><h3>Bottiglie</h3>' +
'        <div class="row"><button id="btnNewUpdateOrder">Aggiorna ordine</button><span class="muted">Scrive le quantità sul foglio e ricarica "Ordine (base)".</span></div>' +
'        <div id="newBottles" style="margin-top:10px;"></div>' +
'      </div>' +
'      <div class="row" style="margin-top:12px;"><span class="spacer"></span><button id="btnNewCancel" class="danger">Cancella</button><button id="btnNewConfirm">Conferma ordine</button></div>' +
'    </div>' +
'  </div>' +

'  <div class="modalBack" id="logBack">' +
'    <div class="modal" role="dialog" aria-modal="true" aria-label="Log">' +
'      <div class="modalHeader">' +
'        <strong>Log</strong>' +
'        <span class="spacer"></span>' +
'        <button id="btnLogClear">Pulisci</button>' +
'        <button id="btnLogClose">Chiudi</button>' +
'      </div>' +
'      <div class="card" style="margin-top:12px;">' +
'        <textarea id="logText" style="width:100%;min-height:55vh;font-family:ui-monospace,Menlo,Consolas,monospace;"></textarea>' +
'      </div>' +
'    </div>' +
'  </div>' +
...
'var elAppVersion=document.getElementById("appVersion");' +
'var elCampaign=document.getElementById("campaign");' +
'var elClient=document.getElementById("client");' +
'var elStatus=document.getElementById("status");' +
'var elCacheInfo=document.getElementById("cacheInfo");' +
'var tblAnag=document.getElementById("tblAnag");' +
'var tblOrdBase=document.getElementById("tblOrdBase");' +
'var tblOrdRange=document.getElementById("tblOrdRange");' +
'var elBottles=document.getElementById("bottles");' +
'var qCognome=document.getElementById("qCognome");' +
'var qMail=document.getElementById("qMail");' +
'var qId=document.getElementById("qId");' +
'var qTel=document.getElementById("qTel");' +
'var btnPrev=document.getElementById("btnPrev");' +
'var btnNext=document.getElementById("btnNext");' +
'var btnNewOrder=document.getElementById("btnNewOrder");' +
'var elMacroBtns=document.getElementById("macroBtns");' +

'var btnShowLog=document.getElementById("btnShowLog");' +
'var logBack=document.getElementById("logBack");' +
'var btnLogClose=document.getElementById("btnLogClose");' +
...
'var btnSearchMautic=document.getElementById("btnSearchMautic");' +
'var newResults=document.getElementById("newResults");' +
'var btnUseSelectedMautic=document.getElementById("btnUseSelectedMautic");' +
'var newRowInfo=document.getElementById("newRowInfo");' +
'var tblNewAnag=document.getElementById("tblNewAnag");' +
'var tblNewOrdBase=document.getElementById("tblNewOrdBase");' +
'var newBottles=document.getElementById("newBottles");' +
'var btnNewUpdateOrder=document.getElementById("btnNewUpdateOrder");' +
'var btnNewCancel=document.getElementById("btnNewCancel");' +
'var btnNewConfirm=document.getElementById("btnNewConfirm");' +

'var MACROS=[' +
  '{name:"fillMauticCustomerData",label:"Aggiorna anagrafica"},' +
  '{name:"creaRiepilogo",label:"Invia riepilogo ordine"},' +
  '{name:"aggiornaSaldoVipselezionati",label:"Saldo VIP"},' +
  '{name:"calcolaLogisticadaSpedizione",label:"Spese Logistica"},' +
  '{name:"createOrderInvoice",label:"Fattura"},' +
  '{name:"creaSpedizioniFromMautic",label:"Crea Spedizione SCOT"},' +
  '{name:"checkGiacenzeSpedizioni",label:"Verifica Giacenze"},' +
  '{name:"notificaSpedizioni",label:"Notifica Spedizione"},' +
  '{name:"processaFileSpedizioni",label:"Invia Spedizioni a SCOT"},' +
  '{name:"metaCapiSendFromSelectedRows",label:"Purchase Meta CAPI"}' +
'];' +

'var allClients=[]; var viewClients=[]; var detailsCache=new Map();' +
'var cacheReady=false, cacheStarted=false, cacheCampaign="";' +
'var qtyTimer=null, pendingQty=null; var cellTimers=new Map();' +
'var newOrder={created:false,rowIndex:0,details:null,bottlesDraft:[]};' +
'var mauticSearchResults=[];' +

'function setStatus(m){ elStatus.textContent=m||""; }' +
'function setModalStatus(m){ modalStatus.textContent=m||""; }' +

'function escapeHtml(s){ s=String(s===undefined||s===null?"":s); return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/\\x27/g,"&#039;"); }' +
'function normPhone(s){ s=String(s||"").trim(); if(!s)return ""; s=s.replace(/[^\\d+]/g,""); s=s.replace(/^(\\+|00)?39/,""); s=s.replace(/[^\\d]/g,""); return s; }' +

'function refreshLogStatus(){ google.script.run.withSuccessHandler(function(r){ if(!r||r.ok===false) return; var on=!!r.hasLog; btnShowLog.disabled=!on; if(on) btnShowLog.classList.add("logOn"); else btnShowLog.classList.remove("logOn"); }).apiGetLogStatus(); }' +
'function openLog(){ logBack.style.display="flex"; loadLog(); }' +
'function closeLog(){ logBack.style.display="none"; }' +
'function loadLog(){ google.script.run.withSuccessHandler(function(r){ if(!r||r.ok===false) return; logText.value=String(r.text||""); }).apiGetLog(); }' +
'function clearLog(){ google.script.run.withSuccessHandler(function(r){ if(!r||r.ok===false) return; logText.value=""; refreshLogStatus(); }).apiClearLog(); }' +

'function apiOk_(r){ return r && r.ok !== false; }' +
'function apiErr_(r){ return (r && r.ok===false) ? String(r.error||"Errore") : "Errore"; }' +
'function pingLog_(){ try{ refreshLogStatus(); }catch(e){} }' +

'function applyClientFilter(){' +
' var fC=String(qCognome.value||"").trim().toLowerCase();' +
' var fM=String(qMail.value||"").trim().toLowerCase();' +
' var fI=String(qId.value||"").trim().toLowerCase();' +
' var fT=normPhone(qTel.value||"");' +
' viewClients=allClients.filter(function(c){' +
'   if(fC && String((c._k&&c._k.cognome)||"").indexOf(fC)<0) return false;' +
'   if(fM && String((c._k&&c._k.mail)||"").indexOf(fM)<0) return false;' +
'   if(fI && String((c._k&&c._k.id)||"").indexOf(fI)<0) return false;' +
'   if(fT && String((c._k&&c._k.tel)||"").indexOf(fT)<0) return false;' +
'   return true;' +
' });' +
' renderClientSelect(true);' +
'}' +

'function renderClientSelect(keep){' +
' var cur=keep?String(elClient.value||""):"";' +
' elClient.innerHTML=""; elClient.appendChild(new Option("— Seleziona —",""));' +
' viewClients.forEach(function(c){ elClient.appendChild(new Option(c.display,String(c.rowIndex))); });' +
' if(cur){ var ok=viewClients.some(function(c){return String(c.rowIndex)===cur;}); if(ok) elClient.value=cur; else clearDetails(); }' +
'}' +

'function clearDetails(){ tblAnag.innerHTML=""; tblOrdBase.innerHTML=""; tblOrdRange.innerHTML=""; elBottles.innerHTML=""; }' +
'function isBooleanField(h){ return String(h||"").indexOf("?")>=0; }' +
'function looksLikeDateValue(v){ v=String(v||"").trim(); if(!v) return false; if(/^\\d{4}-\\d{2}-\\d{2}$/.test(v)) return true; if(/^\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-]\\d{2,4}$/.test(v)) return true; return false; }' +
'function isDateField(h,v){ h=String(h||"").toLowerCase(); if(h.indexOf("data")>=0 || h.indexOf("date")>=0 || h.indexOf("nascita")>=0 || h.indexOf("scadenza")>=0) return true; return looksLikeDateValue(v); }' +
'function toIsoDateForInput(v){ v=String(v||"").trim(); if(!v) return ""; if(/^\\d{4}-\\d{2}-\\d{2}$/.test(v)) return v; var m=v.match(/^(\\d{1,2})[\\/\\-](\\d{1,2})[\\/\\-](\\d{2,4})$/); if(!m) return ""; var dd=("0"+m[1]).slice(-2); var mm=("0"+m[2]).slice(-2); var yy=m[3]; if(yy.length===2) yy="20"+yy; return yy+"-"+mm+"-"+dd; }' +
'function fromIsoToItDate(v){ v=String(v||"").trim(); if(!/^\\d{4}-\\d{2}-\\d{2}$/.test(v)) return v; var p=v.split("-"); return p[2]+"/"+p[1]+"/"+p[0]; }' +
'function normalizeBooleanToSI_NO(v){ v=String(v||"").trim().toLowerCase(); if(!v) return ""; if(v==="si"||v==="sì"||v==="yes"||v==="y"||v==="true"||v==="1") return "SI"; if(v==="no"||v==="n"||v==="false"||v==="0") return "NO"; return ""; }' +

'function renderAnagraficaFixed(d,rowIndex){ var A=d.anagrafica||{}; var sheetUrl=(d._nav&&d._nav.sheetRowUrl)?d._nav.sheetRowUrl:""; var fixed=[["ID Cliente",A["ID Cliente"]],["Cognome",A["Cognome"]],["Nome",A["Nome"]],["VIP",A["VIP"]],["Email",A["Email"]],["Telefono",A["Telefono"]],["Codice Fiscale",A["Codice Fiscale"]],["AD",A["AD"]]]; var stato=A["Stato Pagamento"]||""; var note=A["Note"]||""; var html=""; fixed.forEach(function(kv){ var k=kv[0], v=kv[1]; if(k==="ID Cliente" && sheetUrl){ html+= "<tr><th>"+escapeHtml(k)+"</th><td><a href=\\""+escapeHtml(sheetUrl)+"\\" target=\\"_blank\\" rel=\\"noopener\\">"+escapeHtml(v||"")+"</a></td></tr>"; } else { html+= "<tr><th>"+escapeHtml(k)+"</th><td>"+escapeHtml(v||"")+"</td></tr>"; } }); html+= "<tr><th>Stato Pagamento</th><td class=\\"editableCell\\"><input data-edit=\\"anag\\" data-header=\\"Stato Pagamento\\" data-row=\\""+escapeHtml(rowIndex)+"\\" value=\\""+escapeHtml(stato)+"\\" /></td></tr>"; html+= "<tr><th>Note</th><td class=\\"editableCell\\"><textarea data-edit=\\"anag\\" data-header=\\"Note\\" data-row=\\""+escapeHtml(rowIndex)+"\\">"+escapeHtml(note)+"</textarea></td></tr>"; tblAnag.innerHTML=html; Array.prototype.slice.call(tblAnag.querySelectorAll("[data-edit=\\"anag\\"]")).forEach(function(el){ el.addEventListener("input", function(){ scheduleCellUpdate(rowIndex, el.getAttribute("data-header"), el.value, el, "anagrafica"); }); }); }' +

'function renderOrdineBaseFixed(d){ var O=d.ordineBase||{}; var order=["Prezzo","Spedizione","Voucher","Saldo Logistica","Totale","Totale IMDB","CDV Puro","Ricarico Reale","Ricarico puro"]; tblOrdBase.innerHTML=order.map(function(k){ return "<tr><th>"+escapeHtml(k)+"</th><td>"+escapeHtml(O[k]||"")+"</td></tr>"; }).join(""); }' +

'function renderBottlesExcelEditable(list,rowIndex){ if(!list||!list.length){ elBottles.innerHTML="<span class=\\"muted\\">Nessuna bottiglia trovata.</span>"; return; } var html="<table class=\\"excel\\"><thead><tr><th>Codice bottiglia</th><th>Produttore</th><th>Descrizione</th><th class=\\"qty\\">Numero bottiglie</th></tr></thead><tbody>"; list.forEach(function(b){ html+="<tr><td>"+escapeHtml(b.codice_bottiglia||"")+"</td><td>"+escapeHtml(b.produttore||"")+"</td><td>"+escapeHtml(b.descrizione||"")+"</td>"; html+="<td class=\\"qty editableCell\\"><input type=\\"number\\" min=\\"0\\" step=\\"1\\" data-col=\\""+escapeHtml(b.col)+"\\" value=\\""+escapeHtml(b.numero_bottiglie||"0")+"\\" /></td></tr>"; }); html+="</tbody></table>"; elBottles.innerHTML=html; Array.prototype.slice.call(elBottles.querySelectorAll("input[data-col]")).forEach(function(inp){ inp.addEventListener("input", function(){ var col=Number(inp.getAttribute("data-col")); scheduleQtyUpdate(rowIndex,col,inp.value,inp); }); }); }' +

'function renderDatiAggiuntiviEditable(d,rowIndex){ var list=Array.isArray(d.datiAggiuntiviOrdine)?d.datiAggiuntiviOrdine:[]; var html=""; list.forEach(function(it){ var h=String(it.header||""); var v=(it.value===undefined||it.value===null)?"":String(it.value); if(isBooleanField(h)){ var cur=normalizeBooleanToSI_NO(v); html+="<tr><th>"+escapeHtml(h)+"</th><td class=\\"editableCell\\"><select data-edit=\\"extra\\" data-type=\\"bool\\" data-header=\\""+escapeHtml(h)+"\\">"; html+="<option value=\\"\\"></option><option value=\\"SI\\""+(cur==="SI"?" selected":"")+">SI</option><option value=\\"NO\\""+(cur==="NO"?" selected":"")+">NO</option>"; html+="</select></td></tr>"; } else if(isDateField(h,v)){ var iso=toIsoDateForInput(v); html+="<tr><th>"+escapeHtml(h)+"</th><td class=\\"editableCell\\"><input type=\\"date\\" data-edit=\\"extra\\" data-type=\\"date\\" data-header=\\""+escapeHtml(h)+"\\" value=\\""+escapeHtml(iso)+"\\" /></td></tr>"; } else { html+="<tr><th>"+escapeHtml(h)+"</th><td class=\\"editableCell\\"><input data-edit=\\"extra\\" data-type=\\"text\\" data-header=\\""+escapeHtml(h)+"\\" value=\\""+escapeHtml(v)+"\\" /></td></tr>"; } }); tblOrdRange.innerHTML=html; Array.prototype.slice.call(tblOrdRange.querySelectorAll("[data-edit=\\"extra\\"]")).forEach(function(el){ var type=el.getAttribute("data-type")||"text"; var evt=(type==="date")?"change":"input"; el.addEventListener(evt, function(){ var hdr=el.getAttribute("data-header"); var val=el.value; if(type==="bool") val=String(val||"").trim(); if(type==="date") val=val?fromIsoToItDate(val):""; scheduleCellUpdate(rowIndex,hdr,val,el,"datiAggiuntiviOrdine"); }); }); }' +

'function renderDetails(rowIndex,d){ renderAnagraficaFixed(d,rowIndex); renderOrdineBaseFixed(d); renderBottlesExcelEditable(d.bottiglie,rowIndex); renderDatiAggiuntiviEditable(d,rowIndex); }' +

'function scheduleQtyUpdate(rowIndex,col,qty,el){ pendingQty={rowIndex:String(rowIndex),col:col,qty:qty,el:el}; if(qtyTimer) clearTimeout(qtyTimer); qtyTimer=setTimeout(function(){ var p=pendingQty; pendingQty=null; if(!p) return; applyQtyUpdate(p.rowIndex,p.col,p.qty,p.el); }, 350); }' +
'function applyQtyUpdate(rowIndex,col,qty,el){ var camp=elCampaign.value; if(!camp||!rowIndex) return; if(el) el.classList.add("saving"); google.script.run.withSuccessHandler(function(r){ if(el) el.classList.remove("saving"); if(!apiOk_(r)){ setStatus("Errore bottiglie: "+apiErr_(r)); pingLog_(); return; } if(r && r.ordineBase){ renderOrdineBaseFixed({ordineBase:r.ordineBase}); var d=detailsCache.get(String(rowIndex)); if(d) d.ordineBase=r.ordineBase; } pingLog_(); }).withFailureHandler(function(err){ if(el) el.classList.remove("saving"); setStatus("Errore bottiglie: "+(err.message||err)); pingLog_(); }).apiSetBottleQtyAndGetOrderBase(elCampaign.value,rowIndex,col,qty); }' +

'function scheduleCellUpdate(rowIndex,headerExact,value,el,sec){ var camp=elCampaign.value; if(!camp||!rowIndex) return; var key=String(rowIndex)+"||"+String(headerExact); if(cellTimers.has(key)) clearTimeout(cellTimers.get(key)); if(el) el.classList.add("saving"); var t=setTimeout(function(){ applyCellUpdate(rowIndex,headerExact,value,el,sec); cellTimers.delete(key); }, 450); cellTimers.set(key,t); }' +
'function applyCellUpdate(rowIndex,headerExact,value,el,sec){ var camp=elCampaign.value; if(!camp||!rowIndex) return; google.script.run.withSuccessHandler(function(r){ if(el) el.classList.remove("saving"); if(!apiOk_(r)){ setStatus("Errore salvataggio: "+apiErr_(r)); pingLog_(); return; } var d=detailsCache.get(String(rowIndex)); if(d){ if(sec==="anagrafica"){ d.anagrafica[headerExact]=value; } if(sec==="datiAggiuntiviOrdine" && Array.isArray(d.datiAggiuntiviOrdine)){ for(var i=0;i<d.datiAggiuntiviOrdine.length;i++){ if(String(d.datiAggiuntiviOrdine[i].header)===String(headerExact)){ d.datiAggiuntiviOrdine[i].value=value; break; } } } } pingLog_(); }).withFailureHandler(function(err){ if(el) el.classList.remove("saving"); setStatus("Errore salvataggio: "+(err.message||err)); pingLog_(); }).apiSetCellByHeader(camp,rowIndex,headerExact,value); }' +

'function startCache(camp){ if(cacheStarted) return; cacheStarted=true; cacheCampaign=camp; elCacheInfo.textContent="Cache: caricamento…"; google.script.run.withSuccessHandler(function(res){ if(elCampaign.value!==camp) return; detailsCache=new Map(); if(!apiOk_(res)){ cacheReady=false; elCacheInfo.textContent="Cache non disponibile"; pingLog_(); return; } var rows=(res && res.rows)?res.rows:{}; var count=0; Object.keys(rows).forEach(function(k){ detailsCache.set(String(k),rows[k]); count++; }); cacheReady=true; elCacheInfo.textContent="Cache pronta: "+count+" clienti"; pingLog_(); }).withFailureHandler(function(err){ cacheReady=false; elCacheInfo.textContent="Cache non disponibile"; pingLog_(); }).apiGetAllClientDetails(camp); }' +

'function loadClientDetails(isFirst){ var camp=elCampaign.value; var rowIndex=elClient.value; if(!camp||!rowIndex){ clearDetails(); return; } if(cacheReady && cacheCampaign===camp && detailsCache.has(String(rowIndex))){ renderDetails(rowIndex, detailsCache.get(String(rowIndex))); return; } setStatus("Carico dettagli…"); google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setStatus("Errore dettagli: "+apiErr_(r)); pingLog_(); return; } var d=r.details||r.data||r; renderDetails(rowIndex,d); detailsCache.set(String(rowIndex),d); setStatus(""); if(isFirst) startCache(camp); pingLog_(); }).withFailureHandler(function(err){ setStatus("Errore dettagli: "+(err.message||err)); pingLog_(); }).apiGetClientDetails(camp,rowIndex); }' +

'function loadClients(applyInitialRow){ var camp=elCampaign.value; if(!camp) return; cacheReady=false; cacheStarted=false; cacheCampaign=""; detailsCache=new Map(); setStatus("Carico clienti…"); google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setStatus("Errore clienti: "+apiErr_(r)); pingLog_(); return; } var list=r.clients||[]; allClients=list||[]; viewClients=allClients.slice(); renderClientSelect(false); clearDetails(); if(!allClients.length){ setStatus("Nessun cliente trovato."); pingLog_(); return; } if(applyInitialRow && INITIAL && INITIAL.row){ var rr=String(INITIAL.row); var ok=false; for(var i=0;i<elClient.options.length;i++){ if(elClient.options[i].value===rr){ ok=true; break; } } if(ok){ elClient.value=rr; loadClientDetails(true); pingLog_(); return; } } elClient.selectedIndex=1; loadClientDetails(true); pingLog_(); }).withFailureHandler(function(err){ setStatus("Errore clienti: "+(err.message||err)); pingLog_(); }).apiGetClients(camp); }' +

'function loadCampaigns(){ setStatus("Carico campagne…"); google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setStatus("Errore campagne: "+apiErr_(r)); pingLog_(); return; } var list=r.data||r||[]; elCampaign.innerHTML=""; (list||[]).forEach(function(x){ elCampaign.appendChild(new Option(x.name,x.name)); }); if(!list||!list.length){ setStatus("Nessuna campagna configurata."); pingLog_(); return; } if(INITIAL && INITIAL.camp){ var ex=false; for(var i=0;i<elCampaign.options.length;i++){ if(elCampaign.options[i].value===INITIAL.camp){ ex=true; break; } } if(ex) elCampaign.value=INITIAL.camp; else elCampaign.selectedIndex=0; } else { elCampaign.selectedIndex=0; } setStatus(""); loadClients(true); pingLog_(); }).withFailureHandler(function(err){ setStatus("Errore campagne: "+(err.message||err)); pingLog_(); }).apiGetCampaigns(); }' +

'function moveSelection(delta){ var cur=String(elClient.value||""); if(!cur) return; var idx=-1; for(var i=0;i<viewClients.length;i++){ if(String(viewClients[i].rowIndex)===cur){ idx=i; break; } } if(idx<0) return; var next=idx+delta; if(next<0||next>=viewClients.length) return; elClient.value=String(viewClients[next].rowIndex); loadClientDetails(false); }' +

'function renderMacroButtons(){ elMacroBtns.innerHTML=""; MACROS.forEach(function(m){ var b=document.createElement("button"); b.textContent=m.label; b.title=m.name; b.addEventListener("click", function(){ runMacro(m.name,m.label); }); elMacroBtns.appendChild(b); }); }' +
'function runMacro(name,label){ var camp=elCampaign.value; var rowIndex=elClient.value; if(!camp||!rowIndex) return; setStatus("Eseguo: "+label+"…"); google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setStatus("Errore "+label+": "+apiErr_(r)); pingLog_(); return; } google.script.run.withSuccessHandler(function(rr){ if(!apiOk_(rr)){ setStatus("Errore reload: "+apiErr_(rr)); pingLog_(); return; } var d=rr.details||rr.data||rr; renderDetails(rowIndex,d); detailsCache.set(String(rowIndex),d); setStatus(""); pingLog_(); }).withFailureHandler(function(err){ setStatus("Errore reload: "+(err.message||err)); pingLog_(); }).apiGetClientDetails(camp,rowIndex); }).withFailureHandler(function(err){ setStatus("Errore "+label+": "+(err.message||err)); pingLog_(); }).apiRunMacro(camp,rowIndex,name); }' +

'function openModal(){ resetNewOrder(); modalBack.style.display="flex"; setModalStatus("Inserisci ID Cliente / Cognome / Email e premi Enter"); newQuery.value=""; newQuery.focus(); }' +
'function closeModal(){ modalBack.style.display="none"; }' +
'function resetNewOrder(){ newOrder={created:false,rowIndex:0,details:null,bottlesDraft:[]}; mauticSearchResults=[]; tblNewAnag.innerHTML=""; tblNewOrdBase.innerHTML=""; newBottles.innerHTML=""; newRowInfo.textContent=""; newResults.innerHTML=""; newResults.style.display="none"; btnUseSelectedMautic.style.display="none"; }' +
'function kvTable(el,obj){ var rows=[]; for(var k in obj){ if(Object.prototype.hasOwnProperty.call(obj,k)) rows.push([k,obj[k]]); } el.innerHTML=rows.map(function(kv){ return "<tr><th>"+escapeHtml(kv[0])+"</th><td>"+escapeHtml(kv[1])+"</td></tr>"; }).join(""); }' +

'function handleNewEnter(){ var q=String(newQuery.value||"").trim(); if(!q) return; resetNewOrder(); setModalStatus("Cerco su Mautic…"); google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setModalStatus("Errore Mautic: "+apiErr_(r)); pingLog_(); return; } if(!r || !r.found || !r.results || !r.results.length){ setModalStatus("Cliente non trovato."); pingLog_(); return; } mauticSearchResults = r.results.slice(); if (mauticSearchResults.length === 1) { useSelectedMauticResult_(0); return; } newResults.innerHTML = ""; mauticSearchResults.forEach(function(it, idx) { newResults.appendChild(new Option(it.label, String(idx))); }); newResults.style.display = ""; btnUseSelectedMautic.style.display = ""; if(mauticSearchResults[0] && mauticSearchResults[0].data){ kvTable(tblNewAnag, mauticSearchResults[0].data); } setModalStatus("Trovati " + mauticSearchResults.length + " risultati. Selezionane uno."); pingLog_(); }).withFailureHandler(function(err){ setModalStatus("Errore Mautic: "+(err.message||err)); pingLog_(); }).apiFindMauticCustomerData(q); }' +

'function useSelectedMauticResult_(idx){ idx=Number(idx); if(isNaN(idx) || idx<0 || idx>=mauticSearchResults.length){ setModalStatus("Selezione non valida."); return; } var sel=mauticSearchResults[idx]; var data=sel && sel.data ? sel.data : null; if(!data){ setModalStatus("Dati selezionati non validi."); return; } kvTable(tblNewAnag, data); var baseFields={}; if(data.id) baseFields["ID Cliente"]=data.id; if(data.lastname) baseFields["Cognome"]=data.lastname; if(data.firstname) baseFields["Nome"]=data.firstname; if(data.email) baseFields["Email"]=data.email; if(data.phone) baseFields["Telefono"]=data.phone; if(data.codice_fiscale) baseFields["Codice Fiscale"]=data.codice_fiscale; setModalStatus("Creo la riga ordine…"); google.script.run.withSuccessHandler(function(res){ if(!apiOk_(res)){ setModalStatus("Errore creazione riga: "+apiErr_(res)); pingLog_(); return; } newOrder.created=true; newOrder.rowIndex=res.rowIndex; newOrder.details=res.details; newRowInfo.textContent="Riga creata: "+res.rowIndex; renderNewPopup(res.details); setModalStatus("Riga creata. Imposta bottiglie e conferma."); pingLog_(); }).withFailureHandler(function(err){ setModalStatus("Errore creazione riga: "+(err.message||err)); pingLog_(); }).apiCreateNewOrderRow(elCampaign.value, baseFields); }' +

'function renderNewPopup(details){ var O=details && details.ordineBase ? details.ordineBase : {}; var editable={ "Prezzo":true,"Spedizione":true,"Voucher":true }; var html=""; Object.keys(O).forEach(function(h){ var v=O[h]||""; if(editable[h]) html+="<tr><th>"+escapeHtml(h)+"</th><td class=\\"editableCell\\"><input data-newob=\\"1\\" data-header=\\""+escapeHtml(h)+"\\" value=\\""+escapeHtml(v)+"\\" /></td></tr>"; else html+="<tr><th>"+escapeHtml(h)+"</th><td>"+escapeHtml(v)+"</td></tr>"; }); tblNewOrdBase.innerHTML=html; Array.prototype.slice.call(tblNewOrdBase.querySelectorAll("input[data-newob=\\"1\\"]")).forEach(function(inp){ inp.addEventListener("input", function(){ if(!newOrder.created) return; google.script.run.apiSetCellByHeader(elCampaign.value,newOrder.rowIndex, inp.getAttribute("data-header"), inp.value); pingLog_(); }); }); var list=details && details.bottiglie ? details.bottiglie : []; if(!list.length){ newBottles.innerHTML="<span class=\\"muted\\">Nessuna bottiglia.</span>"; return; } newOrder.bottlesDraft=list.map(function(b){ return {col:b.col, qty:String(b.numero_bottiglie||"0")}; }); var bhtml="<table class=\\"excel\\"><thead><tr><th>Codice bottiglia</th><th>Produttore</th><th>Descrizione</th><th class=\\"qty\\">Numero bottiglie</th></tr></thead><tbody>"; list.forEach(function(b){ bhtml+="<tr><td>"+escapeHtml(b.codice_bottiglia||"")+"</td><td>"+escapeHtml(b.produttore||"")+"</td><td>"+escapeHtml(b.descrizione||"")+"</td>"; bhtml+="<td class=\\"qty editableCell\\"><input type=\\"number\\" min=\\"0\\" step=\\"1\\" data-newqty=\\"1\\" data-col=\\""+escapeHtml(b.col)+"\\" value=\\""+escapeHtml(b.numero_bottiglie||"0")+"\\" /></td></tr>"; }); bhtml+="</tbody></table>"; newBottles.innerHTML=bhtml; Array.prototype.slice.call(newBottles.querySelectorAll("input[data-newqty=\\"1\\"]")).forEach(function(inp){ inp.addEventListener("input", function(){ var col=Number(inp.getAttribute("data-col")); var qty=String(inp.value||"").trim(); if(qty==="") qty="0"; for(var i=0;i<newOrder.bottlesDraft.length;i++){ if(Number(newOrder.bottlesDraft[i].col)===col){ newOrder.bottlesDraft[i].qty=qty; break; } } }); }); }' +

'function newUpdateOrder(){ if(!newOrder.created) return; setModalStatus("Aggiorno bottiglie e ordine base…"); google.script.run.withSuccessHandler(function(res){ if(!apiOk_(res)){ setModalStatus("Errore update: "+apiErr_(res)); pingLog_(); return; } if(res && res.ordineBase && newOrder.details){ newOrder.details.ordineBase=res.ordineBase; renderNewPopup(newOrder.details); } setModalStatus("Aggiornato."); pingLog_(); }).withFailureHandler(function(err){ setModalStatus("Errore update: "+(err.message||err)); pingLog_(); }).apiSetBottleQtyBulkAndGetOrderBase(elCampaign.value,newOrder.rowIndex, newOrder.bottlesDraft); }' +
'function newConfirm(){ closeModal(); loadClients(false); setTimeout(function(){ var r=String(newOrder.rowIndex||""); if(!r) return; elClient.value=r; loadClientDetails(false); }, 700); }' +
'function newCancel(){ if(!newOrder.created){ closeModal(); return; } if(!confirm("Vuoi cancellare la riga "+newOrder.rowIndex+"?")) return; google.script.run.withSuccessHandler(function(r){ if(!apiOk_(r)){ setModalStatus("Errore cancellazione: "+apiErr_(r)); pingLog_(); return; } closeModal(); loadClients(false); pingLog_(); }).withFailureHandler(function(err){ setModalStatus("Errore cancellazione: "+(err.message||err)); pingLog_(); }).apiDeleteRow(elCampaign.value,newOrder.rowIndex); }' +

'if(elAppVersion){ elAppVersion.textContent = INITIAL.version ? ("Versione deployment: " + INITIAL.version) : ""; }' +
'elCampaign.addEventListener("change", function(){ qCognome.value=""; qMail.value=""; qId.value=""; qTel.value=""; loadClients(false); pingLog_(); });' +
'elClient.addEventListener("change", function(){ loadClientDetails(false); pingLog_(); });' +
'[qCognome,qMail,qId,qTel].forEach(function(el){ el.addEventListener("input", applyClientFilter); });' +
'btnPrev.addEventListener("click", function(){ moveSelection(-1); });' +
'btnNext.addEventListener("click", function(){ moveSelection(1); });' +
...
'newQuery.addEventListener("keydown", function(e){ if(e.key==="Enter"){ e.preventDefault(); handleNewEnter(); } });' +
'btnSearchMautic.addEventListener("click", handleNewEnter);' +
'btnUseSelectedMautic.addEventListener("click", function(){ useSelectedMauticResult_(newResults.value); });' +
'newResults.addEventListener("change", function(){ var idx=Number(newResults.value); if(!isNaN(idx) && mauticSearchResults[idx] && mauticSearchResults[idx].data){ kvTable(tblNewAnag, mauticSearchResults[idx].data); } });' +
'btnNewUpdateOrder.addEventListener("click", newUpdateOrder);' +
'btnNewConfirm.addEventListener("click", newConfirm);' +
'btnNewCancel.addEventListener("click", newCancel);' +
'btnShowLog.addEventListener("click", function(){ if(btnShowLog.disabled) return; openLog(); });' +
'btnLogClose.addEventListener("click", closeLog);' +
'btnLogClear.addEventListener("click", function(){ if(!confirm("Vuoi pulire il log?")) return; clearLog(); });' +
'logBack.addEventListener("click", function(e){ if(e.target===logBack) closeLog(); });' +

'renderMacroButtons(); loadCampaigns(); refreshLogStatus(); setInterval(refreshLogStatus, 5000);' +
'</script>' +
'</body></html>'
  );
}
